...
const PurchaseReport = () => {
  ...
  const fetchPurchaseData = async (params = {}) => {
    ...
    try {
      ...
      const response = await api.get(`transaction/purchase-report/branch/${branchId}/?${queryString}`)
      const purchaseData = {
  ...
  const handleDownloadCSV = () => {
    ...
    // Create CSV header
    let csvContent = "Date,Phone,Brand,IMEI,Unit Price,Method\n"

    // Convert each purchase into a CSV row
    data.purchases.forEach((item) => {
      const row = `${item.date},${item.phone},${item.brand},${item.imei_number},${item.unit_price},${item.method}`
      csvContent += row + "\n"
    })

    // Add summary row
    csvContent += `\nSubtotal Purchases: ,,,${data.subtotal_sales}\n`
    csvContent += `Total Purchases: ,,,${data.total_sales}\n`
    csvContent += `Cash Purchases: ,,,${data.cash_sales}\n`
  ...
  const handleDownloadPDF = () => {
    ...
    const headers = [["Date", "Phone", "Brand", "IMEI", "Unit Price", "Method"]]
    ...
    tableData.push(["", "", "", "Subtotal Purchases", data.subtotal_sales])
    tableData.push(["", "", "", "Total Purchases", data.total_sales])
    tableData.push(["", "", "", "Cash Purchases", data.cash_sales])
