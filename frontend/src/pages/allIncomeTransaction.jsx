...
export default function AllIncomeTransactionPage() {
  const api = useAxios()
  ...
  const fetchInitData = async () => {
    try {
      const response = await api.get(`alltransaction/income-transaction/branch/${branchId}/`)
      setRows(response.data.results)
      setMetadata({
        next: response.data.next,
        previous: response.data.previous,
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await api.get(`alltransaction/income-transaction/branch/${branchId}/?search=${localSearchTerm}`)
      setRows(response.data.results)
      setMetadata({
        next: response.data.next,
        previous: response.data.previous,
      ...
      setError('Failed to search income transactions')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDateSearch = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await api.get(`alltransaction/income-transaction/branch/${branchId}/?start_date=${startDate}&end_date=${endDate}`)
      setRows(response.data.results)
      setMetadata({
        next: response.data.next,
        previous: response.data.previous,
      ...
      setError('Failed to filter income transactions by date')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-slate-900 to-slate-800">
      ...
      <div className="flex-grow p-4 px-8 lg:p-6 lg:ml-64">
        ...
        >
          <h1 className="text-3xl lg:text-4xl font-bold text-white mb-4 lg:mb-0">Income Transactions</h1>
        ...
        <div className="mb-6 space-y-4 lg:space-y-0 lg:flex lg:flex-wrap lg:items-center lg:gap-4">
          <form onSubmit={handleSearch} className="w-full lg:w-auto">
            <div className="relative">
              ...
              <Input type="text" placeholder="Search by type, method or description..." value={localSearchTerm} onChange={(e) => setLocalSearchTerm(e.target.value)} className="pl-10 w-full lg:w-64 bg-slate-700 text-white border-gray-600 focus:border-purple-500 focus:ring-purple-500" />
            </div>
          </form>
        ...
        <div className="space-y-6">
          {rows.length > 0 ? (
            rows.map((inc) => (
              <Card key={`${inc.id}-${inc.date}`} onClick={() => {
                navigate(`/income-transaction/branch/${branchId}/edit/${inc.id}`)
                ...
                <CardHeader className="border-b border-slate-700">
                  <CardTitle className="text-lg lg:text-xl font-medium text-white flex flex-col lg:flex-row justify-between items-start lg:items-center">
                    <div>
                      ...
                      <p className='text-sm text-gray-400'>Type: {inc.income_type} &middot; Method: {inc.method}</p>
                    </div>
                    <span className="mt-2 lg:mt-0 text-sm lg:text-base">{format(new Date(inc.date), 'dd MMM yyyy')}</span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-4">
                  <div className="mb-2 p-3 lg:p-4 bg-slate-800 rounded-lg">
                    <div className="text-white font-medium">{inc.desc || 'No description'}</div>
                    <div className="flex justify-between items-center text-sm text-slate-300 mt-2">
                      <span className="font-bold text-l text-green-400">Amount: RS. {inc.amount?.toLocaleString()}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="text-center text-white">No income transactions found.</div>
          )}
        </div>
      ...
      <Button className="fixed bottom-8 right-8 rounded-full w-14 h-14 lg:w-16 lg:h-16 shadow-lg bg-purple-600 hover:bg-purple-700 text-white" onClick={() => navigate(`/income-transaction/form/branch/${branchId}`)}>
